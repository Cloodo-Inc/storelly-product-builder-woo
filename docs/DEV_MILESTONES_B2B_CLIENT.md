...
| **M3** | **Reorder** | F4 | Free | `SPBWC_B2B_Reorders`: My-Account `reorders` endpoint (cards from completed/processing/on-hold orders' `_pcpb_folder` lines, dedup by folder, HPOS-safe `wc_get_orders`) + per-card qty + one-click Reorder → clone folder (independent COW instance) + rebuild `pcpb_meta` + add_to_cart → cart. M2 tier price applies automatically. No pricing-engine changes. Verified: collect skips drafts, reorder clones independent folder, adds to cart. | **done (2026-06-04)** |
| **M4** | **Per-company price overrides** | F2 | Free | `SPBWC_B2B_Price_Rules` + table `{$prefix}spbwc_b2b_price_rules` (idempotent dbDelta, db-version guard; `drop_table()` helper, no uninstall.php to match plugin's data-preserving convention). `SPBWC_B2B_Pricing` refactored to a per-product **cascade**: `resolve_unit_price()` = per-company fixed > per-company pct > tier % > retail (min_qty + valid_until gated). apply_cart_discount + price_html use it. Company detail → "Per-company product pricing" section: bound table + bind/unbind + allow-list sync. Verified: fixed beats tier on cart (42.50 vs 84.92), pct, min_qty gate. Product visibility "B2B only" deferred. | **done (2026-06-04)** |
| **M5** | **Team procurement** | F5 | Free | `SPBWC_B2B_Team`: My-Account `team` endpoint — roster, invite (email+role), edit role + per-order limit, remove, revoke, tokenised **accept-invite** (link logged-in user). `SPBWC_B2B_Procurement`: cart gate (`order_needs_approval` on requester/viewer over per-order limit or company threshold) → "Submit for approval" snapshots cart (incl. pcpb_meta) into headless CPT `spbwc_procurement` (own status meta, not the quote list) + empties cart + notifies approvers; My-Account `approval` queue → approve (builds a WC order for the requester, **cloned design folders**, on-hold) / reject + email. **Note:** uses a dedicated CPT not the `spbwc_quote` state machine (whose transitions don't fit procurement) — timeline pattern reused. Verified: gating, invite/accept→approver, approve→order #for requester with cloned folder. | **done (2026-06-04)** |
| **M6** | **Compliance + tests** | — | Free | **POT regen** (make-pot — all new B2B strings captured) + **RTL** (`b2b-rtl.css` + `b2b-admin-rtl.css` via rtlcss, served on `is_rtl()` for storefront + admin). **Compliance audit of the B2B module against PCP sniff categories: input sanitization (every `$_POST/$_GET` is `isset` + `wp_unslash` + `sanitize_*`), output escaping (`esc_*`/`wp_kses` or pre-escaped helper + phpcs:ignore — no bare echo), nonce + capability/role on every action, `$wpdb->prepare()` on all new queries, consistent text domain, proper enqueue — clean.** No external service. Note: the full-plugin automated `wp plugin check` is too slow to complete in the local Docker env (huge plugin) — run it in CI / a lighter env before release. | **done (2026-06-05)** |
