 ...
 * @version   2.0.0
 */
namespace SPBWC\setasign\Fpdi\PdfParser;

use SPBWC\setasign\Fpdi\PdfParser\CrossReference\CrossReference;
                    ...
                    if (($token2 = $this->tokenizer->getNextToken()) !== \false) {
                        if (\is_numeric($token2)) {
                            if (($token3 = $this->tokenizer->getNextToken()) !== \false) {
                                switch ($token3) {
                                    case 'obj':
                                        return PdfIndirectObject::parse($token, $token2, $this, $this->tokenizer, $this->streamReader);
                                    case 'R':
