...
| **M2** | **Tier pricing** | F3 | Free | `spbwc_b2b_tiers` option + **B2B Pricing** admin page (tier ladder card, add/edit, config-save pattern). Assign tier to company (3 entry points → `_spbwc_company_tier`). Price filter: `woocommerce_product_get_price` applies tier % for company members; stacks with existing `quantity_breaks`. B2B-vs-B2C price display (Pattern 7). **Impact report first** (cart/pricing). | todo |
