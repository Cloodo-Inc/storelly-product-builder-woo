...
if (!class_exists('SPBWC_Storelly_PB_Download_Image')) {
    class SPBWC_Storelly_PB_Download_Image {
        private $url = '';
        private $attachment_data = array();
        private $attachment_id = false;
        public function __construct($url, $attachment_data = array()) {
            $this->url = $this->spbwc_format_url($url);
            if (is_array($attachment_data) && $attachment_data) {
                $this->attachment_data = array_map('sanitize_text_field', $attachment_data);
            }
        }
        private function spbwc_format_url($url) {
            if ($this->spbwc_has_valid_scheme($url)) {
                return $url;
            }
            if ($this->spbwc_does_string_start_with_substring($url, '//')) {
                return "http:{$url}";
            }
            return "http://{$url}";
        }
        private function spbwc_has_valid_scheme($url) {
            return $this->spbwc_does_string_start_with_substring($url, 'https://') || $this->spbwc_does_string_start_with_substring($url, 'http://');
        }
        private function spbwc_does_string_start_with_substring($string, $substring) {
            return 0 === strpos($string, $substring);
        }
        public function spbwc_download() {
            if (!$this->spbwc_is_url_valid()) {
                return false;
            }
            // Download remote file and spbwc_sideload it into the uploads directory.
            $file_attributes = $this->spbwc_sideload();
            if (!$file_attributes) {
                return false;
            }
            // Insert the image as a new attachment.
            $this->spbwc_insert_attachment($file_attributes['file'], $file_attributes['type']);
            if (!$this->attachment_id) {
                return false;
            }
            $this->spbwc_update_metadata();
            $this->spbwc_update_post_data();
            $this->spbwc_update_alt_text();
            return $this->attachment_id;
        }
        private function spbwc_is_url_valid() {
            $parsed_url = wp_parse_url($this->url);
            return $this->spbwc_has_valid_scheme($this->url) && $parsed_url && isset($parsed_url['host']);
        }
        private function spbwc_sideload() {
            // Gives us access to the download_url() and wp_handle_spbwc_sideload() functions.
            ...
            if (!$this->spbwc_is_supported_image_type($mime_type)) {
                return false;
            }
            // An array similar to that of a PHP `$_FILES` POST array
            $file = array(
                'name'     => $this->spbwc_get_filename($mime_type),
                'type'     => $mime_type,
                'tmp_name' => $temp_file,
                'error'    => 0,
            ...
            if ($this->spbwc_did_a_sideloading_error_occur($file_attributes)) {
                return false;
            }
            return $file_attributes;
        }
        private function spbwc_is_supported_image_type($mime_type) {
            return in_array($mime_type, array('image/jpeg', 'image/gif', 'image/png', 'image/x-icon', 'image/svg+xml'), true);
        }
        private function spbwc_get_filename($mime_type) {
            if (empty($this->attachment_data['title'])) {
                return basename($this->url);
            }
            $filename = sanitize_title_with_dashes($this->attachment_data['title']);
            $extension = $this->spbwc_get_extension_from_mime_type($mime_type);
            return $filename . $extension;
        }
        private function spbwc_get_extension_from_mime_type($mime_type) {
            $extensions = array(
                'image/jpeg'    => '.jpg',
                'image/gif'     => '.gif',
                'image/png'     => '.png',
        ...
        private function spbwc_did_a_sideloading_error_occur($file_attributes) {
            return isset($file_attributes['error']);
        }
        private function spbwc_insert_attachment($file_path, $mime_type) {
        ...
        private function spbwc_update_metadata() {
        ...
        private function spbwc_update_post_data() {
        ...
        private function spbwc_update_alt_text() {
