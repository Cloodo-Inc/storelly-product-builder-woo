var in_quick_view = 0;
nbds_frontend = option_builder_variable.nbds_frontend;
var nbOption = {
    status: false,
    initialed: false,
    options: option_builder_variable.options,
    nbd_fields: {},
    extraOdOption: {},
    lastOdOption: {},
    lastExtraOdOption: {},
...
nboApp.controller('optionCtrl', ['$scope', '$timeout', function($scope, $timeout) {
    $scope.product_id = option_builder_variable.product_id;
    $scope.options = nbOption.options;
    $scope.fields = $scope.options["fields"];
    $scope.price = "";
    $scope.type = option_builder_variable.type;
    $scope.variations = option_builder_variable.variations;
    $scope.form_values = option_builder_variable.form_values;
    $scope.is_sold_individually = option_builder_variable.is_sold_individually;
    $scope._quantity = option_builder_variable.quantity;
    $scope.ajax_url = option_builder_variable.ajaxUrl;
                    ...
                    if (file) {
                        ...
                        if (scope.maxsize != '') {
                            ...
                            if (max_size < file.size) {
                                alert(option_builder_variable.file_too_big + scope.maxsize + 'MB');
                                resetInput();
                            }
                        }
                        if (scope.minsize != '') {
                            ...
                            if (minsize > file.size) {
                                alert(option_builder_variable.file_too_small + scope.minsize + 'MB');
                                resetInput();
                            }
                        }
                        if (scope.types != '') {
                            ...
                            if (!checType) {
                                alert(option_builder_variable.file_type + scope.types);
                                resetInput();
                            }
                        }
                    }
