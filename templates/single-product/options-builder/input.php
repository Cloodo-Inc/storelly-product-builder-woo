...
<div class="nbd-option-field pcpb-field-input-wrap <?php echo esc_attr( $class ); ?>"
    ng-if="nbd_fields['<?php echo esc_attr( $field['id'] ); ?>'].enable">
    <?php include( $currentDir .'/options-builder/field-header.php' ); ?>
    <div class="pcpb-field-content">
        <input ng-change="check_valid()" ng-model="nbd_fields['<?php echo esc_attr( $field['id'] ); ?>'].value"
            class="nbd-input-<?php echo esc_attr( $field['general']['input_type'] ); ?>"
            <?php if( $field['general']['required'] == 'y' ) echo esc_attr('required'); ?>
            name="nbd-field[<?php echo esc_attr( $field['id'] ); ?>]"
            id="pcpb-field-<?php echo esc_attr( $field['id'] ); ?>"
            <?php if( $field['general']['input_type'] == 't' ): ?> type="text"
            <?php if( $field['general']['text_option']['min'] != '' ): ?>pattern=".{0}|.{<?php echo esc_attr( $field['general']['text_option']['min'] ); ?>,}"
            <?php endif; ?>
            <?php if( $field['general']['text_option']['max'] != '' ): ?>maxlength="<?php echo esc_attr( $field['general']['text_option']['max'] ); ?>"
            <?php endif; ?>
            <?php if( isset( $field['general']['placeholder'] ) && $field['general']['placeholder'] != '' ): ?>
            placeholder="<?php echo esc_attr( $field['general']['placeholder'] ); ?>" <?php endif; ?>
            <?php elseif( $field['general']['input_type'] == 'u' ): ?> type="file" nbo-input-file="check_valid()"
            data-field-id="<?php echo esc_attr( $field['id'] ); ?>"
            data-types="<?php echo esc_attr(strtolower( trim( $field['general']['upload_option']['allow_type'] ) )); ?>"
            data-minsize="<?php echo esc_attr( $field['general']['upload_option']['min_size'] ); ?>"
            data-maxsize="<?php echo esc_attr( $field['general']['upload_option']['max_size'] ); ?>" <?php 
                    $file_url = '';
                    $filename = '';
                    $uploaded = 0;
        ...
        ?>
        <input class="nbd-upload-hidden" id="nbd-upload-hidden-<?php echo esc_attr( $field['id'] ); ?>" type="hidden"
            name="nbd-field[<?php echo esc_attr( $field['id'] ); ?>]"
