...
| **M2** | **Admin: pricing reply + list** | Custom Quotes `WP_List_Table` (replaces History tab) + 2-col detail screen: request recap, **line-item builder** (custom repeater, editable unit price, live totals), terms (validity/payment/notes), Save draft / **Send pricing reply** / Send counter-offer / Withdraw. Capability + nonce. | todo |
| **M3** | **Storefront submit → CPT** | Rework `Get Quote` submit (`spbwc_submit_quote`) to create a `spbwc_quote` post (replacing WC-order creation). Modal redesign (enqueued CSS, a11y, product context, per-field errors). Keep nonce + nopriv. | todo |
