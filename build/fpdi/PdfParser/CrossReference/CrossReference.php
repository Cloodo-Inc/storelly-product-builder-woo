...
class CrossReference
{
    ...
    public function getIndirectObject($objectNumber)
    {
        ...
        if (\false === $object || !$object instanceof PdfIndirectObject) {
            throw new CrossReferenceException(\sprintf('Object (id:%s) not found at location (%s).', $objectNumber, $offset), CrossReferenceException::OBJECT_NOT_FOUND);
        }
    ...
    protected function findStartXref()
    {
        ...
        if (!$value instanceof PdfNumeric) {
            throw new CrossReferenceException('Invalid data after startxref keyword.', CrossReferenceException::INVALID_DATA);
        }
        return $value->value;
    }
}
